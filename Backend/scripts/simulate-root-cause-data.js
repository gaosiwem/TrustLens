import { PrismaClient, SentimentLabel, SentimentSourceType, } from "@prisma/client";
...
async function main() {
    ...
    console.log(`🚀 Simulating Sentiment Events (Root Cause Data) for brand: ${brandName} (${brand.id})`);
    ...
    for (const issue of systemicIssues) {
        for (let i = 0; i < issue.count; i++) {
            const label = issue.labels[Math.floor(Math.random() * issue.labels.length)];
            ...
            await prisma.sentimentEvent.create({
                data: {
                    ...
                    model: "gpt-4o",
                    provider: "openai",
                    createdAt: new Date(Date.now() - Math.random() * 20 * 24 * 60 * 60 * 1000), // Random time in last 20 days
                },
            });
        }
...
//# sourceMappingURL=simulate-root-cause-data.js.map