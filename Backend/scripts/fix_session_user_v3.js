...
async function main() {
    ...
    // 1. Rename existing user with this email if exists but has different ID
    const existingUser = await prisma.user.findUnique({ where: { email } });
    if (existingUser && existingUser.id !== staleUserId) {
        console.log("Renaming existing user with same email but different ID:", existingUser.id);
        await prisma.user.update({
            where: { id: existingUser.id },
            data: { email: `old_${Date.now()}_${email}` },
        });
    }
...
//# sourceMappingURL=fix_session_user_v3.js.map